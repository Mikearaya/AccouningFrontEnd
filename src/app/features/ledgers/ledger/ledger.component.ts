...
export class LedgerComponent implements OnInit {
    ...
    initializeForm(data: JornalEntryViewModel) {
        ...
        data.Entries.map((d) =>
            this.Entries.push(this.initializeEntryDetail(d))
        );
    ...
    onSubmit() {
        ...
        if (!this.isUpdate) {
            ...
            this.ledgerService
                .updateLedgerEntry(this.ledgerId, formData)
                .subscribe(
                    () => {
                        this.location.back();
    ...
    removeRow(index: number) {
        if (this.Entries.controls[index].get('Id')) {
            const confirmation = confirm(
                'Are you sure u want to delete this entry'
            );
            if (confirmation) {
                this.Entries.removeAt(index);
                this.deletedIds.push(
                    this.Entries.controls[index].get('Id').value
                );
            }
        }
        this.Entries.removeAt(index);
    ...
    prepareData(data: FormGroup): LedgerEntry {
        ...
        this.Entries.controls.forEach((element) => {
            ledger.Entries.push({
                ...
                Debit: element.get('Debit').value
                    ? element.get('Debit').value
                    : 0,
                Credit: element.get('Credit').value
                    ? element.get('Credit').value
                    : 0,
